...
def ocr_page_with_nanonets(image_path: str, max_new_tokens=15000) -> str:
    ...
    prompt = """Extract the text from the above document as if you were reading it naturally and translate to english for none english document. Return the tables in normal format inside <table></table> tags. Return the equations in LaTeX representation. If there is an image in the document and image caption is not present, add a small description of the image inside the <img></img> tag; otherwise, add the image caption inside <img></img>. Watermarks should be wrapped in brackets. Ex: <watermark>OFFICIAL COPY</watermark>. Page numbers should be wrapped in brackets. Ex: <page_number>14</page_number> or <page_number>9/22</page_number>. Prefer using ☐ and ☑ for check boxes."""
    messages = [
